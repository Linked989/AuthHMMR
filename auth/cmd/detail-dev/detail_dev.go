...
func main() {
	...
	for i, dev := range devices {
		...
		fmt.Printf("%d. UUID: %s, Weight: %.2f, Authenticated: %t, LastActive: %s, CorrectVotes: %d, IncorrectVotes: %d, VotesReceivedYes: %d, VotesReceivedNo: %d, VotesReceivedTotal: %d\n",
			i+1, dev.UUID, dev.Weight, dev.Authenticated, lastActive, dev.CorrectVotes, dev.IncorrectVotes, dev.VotesReceivedYes, dev.VotesReceivedNo, dev.VotesReceivedTotal)
	}
